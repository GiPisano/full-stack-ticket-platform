...
@section('head-title', 'Crea nuovo ticket')
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('admin.tickets.store') }}">
                                ...
                                <div class="col-md-6">
                                    <select id="status_id" class="form-control" name="status_id" required>
                                        @foreach ($statuses as $status)
                                            <option value="{{ old('status_id', $status->id) }}">{{ $status->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                    ...
                                    <select id="operator_id" class="form-control" name="operator_id" required>
                                        @foreach ($operators as $operator)
                                            <option value="{{ old('operator_id', $operator->id) }}"
                                                class="{{ $operator->isAvailable() ? 'operator-available' : 'operator-unavailable' }}">
                                ...
                                <div class="col-md-6">
                                    <select id="priority_id" class="form-control" name="priority_id" required>
                                        @foreach ($priorities as $priority)
                                            <option value="{{ old('priority_id', $priority->id) }}"
                                                style="color: {{ $priority->color }}">{{ $priority->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="form-group row">
                                <label for="title" class="col-md-4 col-form-label text-md-right">Titolo del
                                    ticket</label>
